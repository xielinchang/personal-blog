...
class EssayService extends Service {
  ...
  async addReadingNum(body) {
    const { ctx } = this;
    const updated = await ctx.model.Web.BlogEssay.update(
      {
        reading_num: Sequelize.literal('reading_num + 1'), // 使用 sequelize.literal 执行计算操作
      },
      {
        where: {
          id: body.id,
