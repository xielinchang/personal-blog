...
<script>
import myBubble from '../joy-design/my-bubble.vue'
    ...
    initBg() {
      bg().then(res => {
        ...
        if (this.isday >= 5 && this.isday < 17) {
          this.bgimg = import.meta.env.VITE_APP_BASE_API + res.data[0].url
        } else if (this.isday >= 17 && this.isday <= 18) {
          this.bgimg = import.meta.env.VITE_APP_BASE_API + res.data[1].url
        } else {
          this.bgimg = import.meta.env.VITE_APP_BASE_API + res.data[2].url
        }
      })
    },
    addzero(item) {
