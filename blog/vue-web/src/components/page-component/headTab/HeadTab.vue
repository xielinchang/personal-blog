<template>
  <div>
    <header v-show="$store.state.isHead" class="header">
      <div class="header-box">
        ...
        <div class="user-center">
          <div v-if="userInfo.name">
            <div class="user-portrait" right-title="点击打开用户页">
              <router-link to="/user">
                <img
            ...
            <ul v-show="logoutShow" class="logout">
              <li ref="box" @click="logoutUser()">退出登录</li>
            </ul>
            <div v-if="userInfo.name" class="user-name" style="font-size: 14px">
              {{ userInfo.name }}
            </div>
          </div>
        ...
        <div class="writing-center">
          <a target="blank" :href="controllerUrl">创作中心</a>
        </div>
      </div>
    </header>
  </div>
