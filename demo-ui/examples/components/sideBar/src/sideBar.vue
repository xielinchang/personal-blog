<template>
  <div>
    <div class="tabs-nav-wrap" name="header" ref="navWrap">
      ...
      <div
        class="side-bar"
        v-for="(item, index) in data.sidebarList"
        :key="index"
        @click="handleClick($event, index)"
        :class="[{ active: data.current == index }, { disabledStyle: findDisabled(index) }]"
        :style="{ '--margin-bottom': marginBottom + 'px', '--abortColor': abortColor }"
        @disabled="onDisabled"
        @change="change"
      >
...
<script lang="ts" setup>
  ...
  onMounted(() => {
    ...
    for (let i = 0; i < length; i++) {
      ...
      if (itemData[i].props.disabled !== undefined) {
        data.disabledId.push(i);
      }
    }
  });
  ...
  const findDisabled = (e: number) => {
    if (data.disabledId.findIndex((item) => item === e) !== -1) {
      return true;
    } else return false;
  };
